...
const FROM = process.env.EMAIL_FROM ?? "Interlinks <hello@interlinks.ng>";
...
export async function sendReceiptEmail(params: {
  ...
  return getResend().emails.send({
    from: FROM,
    to: params.to,
    subject: `Receipt for your Interlinks ${params.plan} subscription`,
    html: `
      <p>Thanks for subscribing to Interlinks ${params.plan}.</p>
...
export async function sendPaymentFailedEmail(params: {
  ...
  return getResend().emails.send({
    from: FROM,
    to: params.to,
    subject: "Your Interlinks payment did not go through",
    html: `
